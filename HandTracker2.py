...
class HandTracker:
    ...
    def lm_postprocess(self, hand, inference):
        ...
        if hand.lm_score > self.lm_score_thresh:  
            ...
            if self.use_world_landmarks:
                ...
                world_landmarks = self.new_query_xyz(
                    self.new_spatial_loc_roi_from_wrist_landmark,
                    hand,
                    1
                )
                hand.world_landmarks = np.array(world_landmarks)
    ...
    def new_spatial_loc_roi_from_wrist_landmark(self, hand, index):
        # zone_size = max(int(hand.rect_w_a / 10), 8)
        zone_size = max(int(1 + hand.rect_w_a / 100 * 5), 8)
        # zone_size = 8
        # print("_-_-_-_")
        # print(zone_size)
        # print(self.crop_w)
        # print(self.pad_h)
        rect_center = dai.Point2f(*(hand.landmarks[index]-np.array((zone_size//2 - self.crop_w, zone_size//2 + self.pad_h))))
        # rect_center = dai.Point2f(*(hand.landmarks[index] - np.array((zone_size//2, zone_size//2))))  # Sin crop_w ni pad_h
        rect_size = dai.Size2f(zone_size, zone_size)
        return dai.Rect(rect_center, rect_size)

    def new_query_xyz(self, spatial_loc_roi_func, hand, index):
        conf_datas = []
        for i in range(21):
            conf_data = dai.SpatialLocationCalculatorConfigData()
            conf_data.depthThresholds.lowerThreshold = 300
            conf_data.depthThresholds.upperThreshold = 800
            conf_data.roi = spatial_loc_roi_func(hand, i)
        ...
        if len(conf_datas) > 0:
            ...
            for i, sd in enumerate(spatial_data):
                world_landmarks.append(np.array([
